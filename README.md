...
CONTROLLER: Currently there is a very simple command line ATDECC controller in the scrambletools/avb_tools repo, for basic connectivity testing. The main application is intended to showcase the talker and listener functionality of AVB, so it requires a controller to make a connection. It has been tested with the Hive AVB controller (<https://github.com/christophe-calmejane/hive>), the avb_tools controller as well as the Apple ATDECC controller (built into MacOS).
